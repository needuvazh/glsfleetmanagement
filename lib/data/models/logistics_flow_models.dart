...
class CustomerRequestModel extends CustomerRequestData {
  ...
  factory CustomerRequestModel.fromMap(Map<String, dynamic> map) {
    final pickup = map['pickup'] as String? ?? map['pickupLocation'] as String? ?? '';
    final drop = map['delivery'] as String? ?? map['drop'] as String? ?? '';
    final weightRaw = map['weightVolume'] ?? map['weight'];
    return CustomerRequestModel(
      customerName: map['customerName'] as String? ?? map['clientName'] as String? ?? '',
      ...
      pickup: pickup,
      delivery: drop,
      date: map['date'] as String? ?? '2026-03-24',
    );
  }
}
...
class FleetVehicleModel extends FleetVehicleData {
  ...
  factory FleetVehicleModel.fromMap(Map<String, dynamic> map) {
    final vehicleNo = map['vehicleNo'] as String? ?? map['vehicleNumber'] as String? ?? '';
    ...
    return FleetVehicleModel(
      ...
      ivmsDeviceId: map['ivmsDeviceId'] as String? ?? 'IVMS-${vehicleNo.replaceAll(' ', '-')}',
      status: map['status'] as String? ?? '',
    );
  }
}
...
class JourneyMasterModel extends JourneyMasterData {
  ...
  factory JourneyMasterModel.fromMap(Map<String, dynamic> map) {
    ...
    final restPoints = (map['restPoints'] as List<dynamic>? ?? const <dynamic>[])
        .map((item) => item.toString())
        .toList();

    final origin = map['origin'] as String? ?? (route.isNotEmpty ? route.first : '');
    final destination = map['destination'] as String? ?? (route.isNotEmpty ? route.last : '');
    final planName = map['planName'] as String? ?? '${origin.isEmpty ? 'Origin' : origin} to ${destination.isEmpty ? 'Destination' : destination}';
    ...
    return JourneyMasterModel(
      ...
      stops: stops,
      restPoints: restPoints.isEmpty ? (stops.length > 1 ? [stops[1]] : const <String>[]) : restPoints,
    );
  }
}
